...
Handles validation and processing of rule schemas.
"""
...
class RuleProcessor(SchemaProcessor):
    ...
    def __init__(self):
        ...
        self.optional_fields = {'examples', 'guidelines', 'standards', 'rules',
    ...
    def validate(self, data: Dict) -> Tuple[bool, List[str], List[str]]:
        ...
        if 'title' in data:
            if not isinstance(data['title'], str):
                ...
                warnings.append("Title should start with a capital letter and contain only letters, numbers, spaces, and hyphens")
        ...
        for field in content_fields:
            if field in data:
                ...
                if not isinstance(data[field], list):
                    errors.append(f"Field '{field}' must be a string or list of strings")
                else:
                    empty_items = [i for i, item in enumerate(data[field])
                                 if not isinstance(item, str) or not item.strip()]
                    if empty_items:
                        errors.append(f"Empty or invalid items in {field}: {empty_items}")
    ...
    def normalize_content(self, content: str) -> str:
        """
        Normalize rule content to standard YAML format.
        Converts various formats to consistent YAML structure.
        """
        # Process the content
        result = self.process(content)
        if not result.is_valid:
            raise ValueError(f"Cannot normalize invalid content: {result.errors}")
        ...
        for field in self.optional_fields:
            if field in result.data:
                normalized[field] = result.data[field]
        
        # Convert guidelines/standards/rules to lists if they're strings
        for field in {'guidelines', 'standards', 'rules'}:
