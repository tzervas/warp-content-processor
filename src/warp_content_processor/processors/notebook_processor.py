...
class NotebookProcessor(SchemaProcessor):
    ...
    def _extract_front_matter(self, content: str) -> Tuple[Optional[Dict], str, List[str]]:
        """
        Extract and parse YAML front matter from notebook content.
        
        Returns:
            Tuple[Optional[Dict], str, List[str]]: (front_matter, remaining_content, errors)
        """
    ...
    def validate(self, data: Dict) -> Tuple[bool, List[str], List[str]]:
        ...
        if not front_matter:
            ...
            if missing_fields:
                errors.append(f"Missing required fields in front matter: {missing_fields}")
            
            # Validate field types
            if 'title' in front_matter and not isinstance(front_matter['title'], str):
                errors.append("Field 'title' must be a string")
            if 'description' in front_matter and not isinstance(front_matter['description'], str):
            ...
            if 'tags' in front_matter:
                if not isinstance(front_matter['tags'], list):
                    ...
                    invalid_tags = [tag for tag in front_matter['tags']
        ...
        if not content.strip():
            ...
            for block in code_blocks:
                ...
                if placeholders:
                    warnings.append(f"Code block contains command placeholders: {placeholders}")
    ...
    def process(self, content: str) -> ProcessingResult:
        ...
        try:
            ...
            front_matter, remaining_content, errors = self._extract_front_matter(content)
            if errors:
