...
class ContentSanitizer:
    ...
    def validate_file_path(file_path: Union[str, Path]) -> Path:
        ...
        try:
            if path.exists() and path.stat().st_size > MAX_CONTENT_SIZE:
                raise SecurityValidationError(f"File size exceeds {MAX_CONTENT_SIZE} bytes")
        except OSError:
    ...
    def sanitize_string(content: str, max_length: int = 100000) -> str:
        ...
        if len(content) > max_length:
            raise SecurityValidationError(f"Content exceeds maximum length of {max_length}")
        ...
        for pattern in DANGEROUS_PATTERNS:
            if re.search(pattern, content, re.IGNORECASE | re.DOTALL):
                raise SecurityValidationError(f"Content contains dangerous pattern: {pattern}")
    ...
    def validate_yaml_structure(data: Union[Dict, List], depth: int = 0) -> None:
        ...
        if isinstance(data, dict):
            if len(data) > MAX_ARRAY_LENGTH:
                raise SecurityValidationError(f"Dictionary size exceeds {MAX_ARRAY_LENGTH}")
