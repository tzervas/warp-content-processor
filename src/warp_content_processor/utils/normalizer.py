...
from .security import ContentSanitizer, secure_yaml_load, secure_yaml_dump
    ...
    def normalize_workflow_content(content: str) -> Dict:
        ...
        if remaining:
            try:
                ...
                workflow.update(ContentNormalizer._extract_workflow_from_text(remaining))
    ...
    def _normalize_workflow_fields(workflow: Dict) -> Dict:
        ...
        if 'shells' in workflow:
            ...
            if isinstance(shells, list):
                normalized['shells'] = [str(shell).strip().lower() for shell in shells if shell]
                ...
                for arg in args:
                    if isinstance(arg, dict):
                        ...
                        if 'description' in arg:
                            normalized_arg['description'] = str(arg['description']).strip()
            ...
            try:
                ...
                if isinstance(yaml_data, dict):
                    ...
                    if content_type == 'workflow':
                        normalized = ContentNormalizer._normalize_workflow_fields(yaml_data)
                    elif content_type == 'prompt':
                        normalized = ContentNormalizer._normalize_prompt_fields(yaml_data)
                    else:
