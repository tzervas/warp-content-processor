    ...
    def split_content(cls, content: str) -> List[Tuple[str, str]]:
        ...
        while current_pos < content_length:
            ...
            for content_type, pattern in cls.DOCUMENT_PATTERNS.items():
                match = re.search(pattern, content[current_pos:], re.MULTILINE | re.DOTALL)
                ...
                if doc_type in self.processors:
                    ...
                    if result.is_valid:
                        ...
                        while output_path.exists():
                            ...
                            output_path = self.output_dir / doc_type / f"{base_name}_{counter}.{ext}"
                            counter += 1
                        
                        output_path.write_text(doc_content)
                        logger.info(f"Saved {doc_type} content to {output_path}")
                    
                    results.append(result)
                else:
                    logger.warning(f"Unknown content type in {file_path}")
