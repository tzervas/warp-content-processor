...
def assert_yaml_structure(data: Dict[str, Any], expected_keys: List[str], message: str = None) -> None:
...
def assert_file_content(file_path: Union[str, Path], expected_content: str, message: str = None) -> None:
...
def assert_security_safe(content: str, dangerous_patterns: List[str] = None, message: str = None) -> None:
...
def assert_validation_passes(data: Dict[str, Any], validator_func, message: str = None) -> None:
...
def assert_cell_structure(cell: Dict[str, Any], message: str = None) -> None:
    ...
    if cell["cell_type"] not in valid_cell_types:
        error_msg = f"Invalid cell type: {cell['cell_type']}. Must be one of {valid_cell_types}"
        if message:
            error_msg = f"{message}: {error_msg}"
        raise AssertionError(error_msg)
...
def assert_processing_result(result: Dict[str, Any], expected_content_types: List[str] = None, message: str = None) -> None:
...
def assert_error_handling(func, expected_exception_type: type, *args, **kwargs) -> None:
    ...
    try:
        func(*args, **kwargs)
        raise AssertionError(f"Expected {expected_exception_type.__name__} to be raised")
    except expected_exception_type:
        # Expected exception was raised
        pass
    except Exception as e:
        raise AssertionError(f"Expected {expected_exception_type.__name__}, but got {type(e).__name__}: {e}")


def assert_log_contains(log_records: List, expected_message: str, level: str = None, message: str = None) -> None:
...
def assert_performance_within_limits(execution_time: float, max_time: float, message: str = None) -> None:
    """Assert that execution time is within acceptable limits."""
    if execution_time > max_time:
        error_msg = f"Execution time {execution_time:.3f}s exceeds limit of {max_time:.3f}s"
        if message:
            error_msg = f"{message}: {error_msg}"
        raise AssertionError(error_msg)


def assert_memory_usage_reasonable(memory_usage: int, max_memory_mb: int, message: str = None) -> None:
...
def assert_string_contains_pattern(text: str, pattern: str, message: str = None) -> None:
...
def assert_lists_equal_unordered(list1: List[Any], list2: List[Any], message: str = None) -> None:
...
def assert_dict_subset(subset: Dict[str, Any], superset: Dict[str, Any], message: str = None) -> None:
    ...
    for key, value in subset.items():
        ...
        if superset[key] != value:
            error_msg = f"Value mismatch for key '{key}': expected {value}, got {superset[key]}"
            if message:
                error_msg = f"{message}: {error_msg}"
            raise AssertionError(error_msg)
